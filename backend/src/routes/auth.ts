...
router.post('/recover-password', (req: Request, res: Response) => {
  ...
  const MASTER_KEY = process.env.MASTER_RECOVERY_KEY || 'ROI-RECOVER-2026';

  if (!username || !recoveryKey || !newPassword) {
    return res.status(400).json({ error: 'Username, recovery key, and new password are required' }) as any;
  }

  if (recoveryKey !== MASTER_KEY) {
